...
const profileData = {
  ...
  education: [
    { degree: "MSc, Computer Science", institution: "Kotebe University of Education", year: "2025" },
    { degree: "MBA, Business Administration", institution: "Gage University College", year: "2022" },
    { degree: "BSc, Computer Science", institution: "Debre Birhan University", year: "2018" },
  ],
  social: {
    linkedin: "https://www.linkedin.com/in/yabibal-zelalem-520989225/",
    github: "https://github.com/Yabibal1624", // Placeholder, adjust if real link is provided
    website: "http://www.scitech.com.et"
  }
};

// Projects Data (Using the most up-to-date Projects.jsx content)
const projectsData = [
  {
...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      {/* Mobile Menu */}
      <motion.div
        className="md:hidden absolute top-full left-0 w-full bg-gray-900 shadow-xl"
        initial={false}
...
const DualExpertiseDashboard = ({ profile }) => {
    ...
    const techSkills = skills.development.filter(s => s !== "Git" && s !== "REST APIs").slice(0, 5);
        ...
        >
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="flex-1 space-y-4">
                    <h4 className="text-lg font-semibold text-teal-300 border-b border-teal-700/50 pb-2">Core Development Focus</h4>
                    ...
                    <h4 className="text-lg font-semibold pt-4 text-fuchsia-300 border-b border-fuchsia-700/50 pb-2">Advanced Education</h4>
                    <p className="text-gray-300 text-base">
                        <span className="font-bold">{latestEducation.degree}</span>, {latestEducation.institution} ({latestEducation.year})
                    </p>
                    <p className="text-gray-400 text-sm italic">
                        The intersection of technical depth and strategic business knowledge (MBA, {dualEducation.year}).
                    </p>
                </div>
                ...
                <div className="flex-1 space-y-4">
                    <h4 className="text-lg font-semibold text-fuchsia-300 border-b border-fuchsia-700/50 pb-2">Systems & Data Integration</h4>
                    <div className="flex flex-wrap gap-2">
                     ...
                     <h4 className="text-lg font-semibold pt-4 text-teal-300 border-b border-teal-700/50 pb-2">Key Competencies</h4>
...
const Hero = ({ profile }) => {
    ...
    const rotateX = useSpring(useTransform(y, [-150, 150], [30, -30]), { stiffness: 400, damping: 20 });
    const rotateY = useSpring(useTransform(x, [-150, 150], [-30, 30]), { stiffness: 400, damping: 20 });
...
const ProjectCard = ({ project }) => (
  <motion.div
    className="project-card bg-gray-800 rounded-xl overflow-hidden shadow-2xl hover:shadow-teal-500/30 transition duration-300 border border-gray-700 hover:border-teal-500"
    variants={fadeInUp}
  ...
  >
    ...
    <div className="p-6">
        ...
        <p className="text-gray-400 text-sm mb-4 min-h-[60px]">{project.description}</p>
...
const Projects = () => {
  return (
    <motion.section
      id="projects"
      className="projects py-20 px-6 md:px-16 bg-gray-950 text-white"
      initial={{ opacity: 0 }}
    ...
    >
      ...
      <motion.div
        className="project-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 max-w-6xl mx-auto"
        variants={staggerContainer}
        initial="initial"
        whileInView="animate"
...
const Contact = () => {
  // NOTE: These IDs are placeholders. In a real React app, you would load these
  // from environment variables (e.g., import.meta.env.VITE_EMAILJS_PUBLIC_KEY).
  // The submission will be simulated as the key is left empty for safety.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        // --- SIMULATED EmailJS SEND ---
        await new Promise(resolve => setTimeout(resolve, 1500)); 
        
        if (EMAILJS_PUBLIC_KEY === "") {
            // Simulate success if key is missing (standard behavior for demos)
            console.warn("EmailJS key is missing. Simulating successful message send.");
        } else {
             // If key were present, uncomment the line below:
  ...
  const EmailIcon = (props) => (<svg {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.902l-2.618 1.621a2.25 2.25 0 00-.742.827l-.774 1.355a1.275 1.275 0 01-1.29 2.05h-.342a1.275 1.275 0 01-1.29-2.05l-.774-1.355a2.25 2.25 0 00-.742-.827l-2.62-1.621A2.25 2.25 0 014.5 6.993V6.75" /></svg>);
  ...
  const LocationIcon = (props) => (<svg {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-6 12-7.5 12S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" /></svg>);

  return (
    <motion.section
      id="contact"
      className="contact py-20 px-6 md:px-16 bg-gray-900 text-white"
      initial={{ opacity: 0 }}
      whileInView={{ opacity: 1 }}
      viewport={{ once: true, amount: 0.1 }}
      transition={{ duration: 0.6 }}
    >
      ...
      <div className="max-w-4xl mx-auto flex flex-col lg:flex-row gap-12">
        {/* Contact Info */}
        <motion.div
          className="contact-info w-full lg:w-1/3 space-y-8 p-6 bg-gray-800 rounded-xl shadow-lg border border-fuchsia-700/50"
          variants={fadeInUp}
          initial="initial"
          whileInView="animate"
          viewport={{ once: true }}
        >
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="font-medium text-teal-300 break-words">yabibal25@gmail.com</p>
            </div>
          </div>
          {/* Phone */}
          <div className="flex items-center space-x-4">
        ...
        >
            <motion.form 
                className="contact-form space-y-6 bg-gray-800 p-8 rounded-xl shadow-2xl" 
            ...
            >
                ...
                <motion.button
                    className="w-full p-4 font-bold text-lg rounded-lg shadow-lg transition duration-300"
                    type="submit"
                    whileHover={{ scale: 1.02, backgroundColor: '#A21CAF' }}
                    whileTap={{ scale: 0.98 }}
                    disabled={formStatus.submitting}
                    style={{ backgroundColor: formStatus.submitting ? '#5B21B6' : '#D946EF' }}
                >
                ...
                {formStatus.message && (
                    <motion.div
                        className={`p-4 rounded-lg font-semibold text-center mt-4 ${
                            formStatus.success ? "bg-teal-500/20 text-teal-300 border border-teal-500" : 
...
// ==========================================================
// 6. MAIN APP COMPONENT & STYLES
// ==========================================================

// Custom styles that ensure Inter font and smooth scrolling
const CustomStyles = () => (
    <style jsx="true">{`
        ...
        /* Ensures smooth scrolling to anchors */
        html {
            scroll-behavior: smooth;
        }

        /* Responsive Hero Text Size Adjustment for smaller screens */
...
function App() {
  ...
  // NOTE: Initializing EmailJS here, but using an empty key as per guidelines. 
  // Actual submission will be simulated in the Contact component.
  const EMAILJS_PUBLIC_KEY = ""; 
  ...
  return (
    <>
      ...
      <div className={`app ${isLoaded ? "loaded" : ""}`}>
        ...
        <motion.footer
          className="footer text-center py-8 bg-gray-950 border-t border-gray-800 text-gray-500"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
        >
          <p className="text-sm"> &copy; {new Date().getFullYear()} Yabibal Zelalem. All rights reserved.</p>
          <p className="text-xs pt-1 text-gray-600">Built with React, Tailwind CSS, & Framer Motion.</p>
        </motion.footer>
      </div>
    </>
  );
